...
print("Testing all the same dropouts for RNN-CNN (with LeakyRelu for constant)")
print(len(training))
...
dropout_percs = [0.1, 0.2, 0.5, 0.0]
overall_test_accuracies = []
num_epochs = 25
epochs = range(num_epochs) + np.ones(num_epochs)

for i in range(len(dropout_percs)):
    train_accuracies = []
    test_accuracies = []
    net = Net(dropout_percs[i]).to(device)
    ...
    #save model
    model_name = "dropout_model_" + str(i) + "_lr_0001_perc_20_epochs.pth"
    torch.save(net.state_dict(),model_name)

    new_name = "train_accuracy_dropout_model_" + str(i) + "_lr_0001_perc_20_epochs.pickle"
    with open(new_name, 'wb') as f:
        pickle.dump(train_accuracies, f)

    new_name = "test_accuracy_dropout_model_" + str(i) + "_lr_0001_perc_20_epochs.pickle"
    with open(new_name, 'wb') as f:
    ...
    plt.savefig("dropout_" + str(i) + "_lr_0001_accuracies_plot.png")
    plt.show()
    plt.close()
...
plt.plot(epochs, overall_test_accuracies[1], color="yellow", linewidth = 2)
...
plt.legend(["10%", "20%", "50%", "0%"], loc = "upper right")
plt.title('Testing Accuracy with Varying Dropouts Over 20 Epochs')

plt.savefig('dropout_lr_0001_all_models_plot.png')
plt.show()